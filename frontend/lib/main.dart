...
class _MainScreenState extends State<MainScreen> {
  ...
  Future<void> _loadPersistedData() async {
    ...
    setState(() {
      ...
      if (savedTime != null) {
        _currentDateTime = DateTime.fromMillisecondsSinceEpoch(
          savedTime,
          isUtc: false,
        );
      }
  ...
  void _onTick(Timer timer) {
    ...
    if (_isPlaying) {
      setState(() {
        _currentDateTime = _currentDateTime.add(
  ...
  void _fetchChartDataIfNeeded({bool force = false}) {
    ...
    if (force ||
        _currentDateTime.difference(_lastFetchedTime).inHours.abs() >= 6) {
      ...
      final timeStr =
          '${fetchStart.hour.toString().padLeft(2, '0')}:${fetchStart.minute.toString().padLeft(2, '0')}';
  ...
  Map<String, dynamic> _interpolateData(
    dynamic rootData,
    DateTime fetchStart,
    ...
    double diffHours =
        currentDateTime.difference(fetchStart).inMilliseconds /
        (1000 * 60 * 60);
    ...
    final newHouses = (c0['houses'] as List<dynamic>).asMap().entries.map((
      entry,
    ) {
      int idx = entry.key;
  ...
  static const _cupertinoTheme = CupertinoThemeData(
    ...
    textTheme: CupertinoTextThemeData(
      dateTimePickerTextStyle: _pickerTextStyle,
    ),
  );
            ...
            onPointerDown: (event) {
              if (_isMapDragging) {
                ...
                final mapRect = Rect.fromLTWH(
                  16,
                  size.height - 16 - bottomInset - mapH,
                  mapW,
                  mapH,
                );
            ...
            child: BlocBuilder<NatalCubit, NatalState>(
              builder: (context, state) {
                if (state is NatalStateEmpty) {
                  ...
                  final interpolatedData = _interpolateData(
                    state.root,
                    state.fetchTime,
                    _currentDateTime,
                  );
            ...
            AnimatedPositioned(
              ...
              child: ClipRRect(
                ...
                child: Container(
                  decoration: BoxDecoration(
                    border: Border.all(
                      color: _isMapDragging ? Colors.white54 : Colors.white24,
                    ),
                    borderRadius: BorderRadius.circular(12),
                  ),
                  child: FlutterMap(
                    options: MapOptions(
                      initialCenter:
                          _selectedLocation ?? const LatLng(51.5, -0.1),
                      initialZoom: 10.0,
  ...
  void _showGeminiSettings() async {
    ...
    final currentModel =
        prefs.getString('gemini_model') ?? 'gemini-2.0-flash-lite';
...
class GeminiTokenTracker {
  ...
  Future<void> load() async {
    ...
    _entries = raw.map((s) {
      final parts = s.split(',');
      return {
        'ts': int.parse(parts[0]),
        'in': int.parse(parts[1]),
...
class _NatalChartViewState extends State<NatalChartView> {
  ...
  void _showInterpretation(String key) async {
    ...
    try {
      ...
      final prompt =
          'Provide a short, insightful astrological interpretation of the symbol/entity: "$key". Keep it under 3 sentences. Do not use markdown.';
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (pinnedKey != null) {
      if (data['angles'].containsKey(pinnedKey)) {
        ...
        var h = (data['houses'] as List).firstWhere(
          (e) => e['num'] == hNum,
          orElse: () => data['houses'][0],
        );
    ...
    for (int i = 0; i < aspectPoints.length; i++) {
      for (int j = i + 1; j < aspectPoints.length; j++) {
        ...
        } else if ((diff - 90).abs() <= 8) {
          // Square
          aspectColor = Colors.red;
          strokeWidth = 2.0;
          aspectName = 'Square';
        } else if ((diff - 120).abs() <= 8) {
          // Trine
          aspectColor = Colors.blue;
          strokeWidth = 1.5;
          aspectName = 'Trine';
        } else if ((diff - 60).abs() <= 6) {
          // Sextile
        ...
        if (aspectColor != null) {
          ...
          bool isHovered =
              hoverPos != null && _distToSegment(hoverPos!, pt1, pt2) < 6.0;
          ...
          if (isHovered) {
            ...
            tooltips.add({
              'text':
                  '${aspectPoints[i]['name']} - ${aspectPoints[j]['name']} ($aspectName)',
              'pos': hoverPos!,
            });
          }
          ...
          canvas.drawLine(
            ...
            pt2,
            Paint()
              ..color = aspectColor.withAlpha(isHovered ? 255 : 150)
    ...
    data['angles'].forEach((k, o) {
      ...
      bool isHovered =
          hoverPos != null && (hoverPos! - symbolPos).distance < 25;
